...
Exécuter d’abord l’algorithme glouton (Greedy), de complexité $O(n^3)$, afin d’obtenir une première solution approximative, jouant le rôle de borne supérieure initiale.
Réordonner les sommets de manière à placer en premier ceux sélectionnés par cette solution gloutonne.
Exécuter ensuite le Branch & Bound classique sur cette instance ré-indexée.
L’intérêt de cette approche est que, si l’ordre proposé par Greedy est pertinent, l’algorithme exact dispose rapidement d’une bonne solution, ce qui permet d’élaguer plus tôt une grande partie de l’arbre de recherche. On s’attend donc à réduire le temps d’exécution ou, au minimum, à diminuer la base exponentielle effective.
La suite présente une comparaison directe entre le Branch & Bound classique et cette version ré-indexée.
```{r}
...
La théorie prédit que le ratio d'approximation est majoré par $\ln(n)$.
Grâce à la performance de notre Branch & Bound C++, nous pouvons calculer l'optimum exact pour $n$ allant jusqu'à 25 et vérifier si le ratio observé reste bien sous cette courbe théorique.
